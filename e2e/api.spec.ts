/**
 * API E2E tests — calls the Cloudflare Worker API directly.
 *
 * When `WORKER_URL` env var is set (e.g. wrangler dev on port 8787) the tests
 * hit the live worker. Otherwise they hit the Vite proxy at localhost:5173/api.
 */
...
test.describe("API — /api/static", () => {
  test("returns salons, services, stylists, categories, provinces", async ({ request }) => {
    const res = await request.get(`${WORKER_URL}/api/static`);
    expect(res.status()).toBe(200);
    const body = await res.json();
...
test.describe("API — /api/slots", () => {
  test("returns array of slots for salonId=1 and today's date", async ({ request }) => {
    const date = todayVN();
    const res = await request.get(`${WORKER_URL}/api/slots?salonId=1&date=${date}`);
    expect(res.status()).toBe(200);
  ...
  test("returns 400 when salonId is missing", async ({ request }) => {
    const res = await request.get(`${WORKER_URL}/api/slots?date=${todayVN()}`);
  ...
  test("returns 400 when date format is invalid", async ({ request }) => {
    const res = await request.get(`${WORKER_URL}/api/slots?salonId=1&date=not-a-date`);
    expect(res.status()).toBe(400);
    const body = await res.json();
    expect(body.ok).toBe(false);
  });
...
test.describe("API — unknown routes", () => {
  test("returns 404 for unknown path", async ({ request }) => {
    const res = await request.get(`${WORKER_URL}/api/unknown-route`);
